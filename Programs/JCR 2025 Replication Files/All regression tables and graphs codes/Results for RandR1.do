...
**********************04052025 Saturday to April 072025 Monday**************************************************************************************************************************
*===============================================================
* Summary Tables for Different Governorates
* Calculating Missing Women (Age 0-1) for Each Year and Governorate
*===============================================================
...
forvalues i = 2007/2014 {
    forvalues j = 1/2 {
        foreach k of numlist 0 1{ //urban or rural
            
            * Calculate the number of people (age 0-1) by gender in each governorate and year
            count if b_gender == `j' & (kidage`i' == 0 | kidage`i' == 1) & urban == `k' & year == `i'
            if `j' == 1 {
                replace pop_male_01 = r(N) if urban == `k' & year == `i'
            }
            else {
                replace pop_female_01 = r(N) if urban == `k' & year == `i'
            }

            * Calculate the number of deaths (age 0-1) by gender in each governorate and year
            count if b_gender == `j' & deathyear == `i' & (dage == 0 | dage == 1) & urban == `k' & b_alive == 0
            if `j' == 1 {
                replace death_male_01 = r(N) if urban == `k' & year == `i'
            }
            else {
                replace death_female_01 = r(N) if urban == `k' & year == `i'
            }
...
* Save the updated dataset for individual channel check
save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\data\data_cleaning\Cleaned DHS\Sum_MF_DeathRate", replace 

* Process for Male Death Rate (rate_male_01)
use "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\data\data_cleaning\Cleaned DHS\Sum_MF_DeathRate", clear

* Drop unnecessary merge variables
drop _merge 
...
* Create a copy of 'governates_ur' as 'tgover_ur1' if needed
gen tgover_ur1 = governates_ur

* Filter the data to include only rows where b_gender == 1 (Male)
keep if b_gender == 1 

* List all variables except the grouping variables for collapse
ds caseid governates_ur year urban b_gender past1 tgover_ur1, not
local varlist ""

* Loop through the variables and exclude the problematic ones
foreach var of varlist `r(varlist)' {
    if "`var'" != "tgover_ur1" & "`var'" != "past1" {
        local varlist `varlist' `var'
    }
}

* Collapse the data by urban and past1
collapse (mean) `varlist', by(urban past1)

* Keep relevant variables and add gender identifier
gen b_gender = 1
keep b_gender urban past1 rate_male_01

* Sort the data and generate a unique ID for merging
sort urban b_gender past1
gen ID = _n

* Label the 'urban' variable with descriptive values
label define urban_label 0 "Rural" 1 "Urban"
label values urban urban_label

* Save the male reshaped dataset
save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_M_DeathRate2_reshaped.dta", replace

* Process for Female Death Rate (rate_female_01)
use "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\data\data_cleaning\Cleaned DHS\Sum_MF_DeathRate", clear
...
* Filter the data to include only rows where b_gender == 2 (Female)
keep if b_gender == 2 
...
* Keep relevant variables and add gender identifier
gen b_gender = 2
keep b_gender urban past1 rate_female_01
...
* Save the female reshaped dataset
save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_F_DeathRate2_reshaped.dta", replace

* Merging Male and Female Datasets by ID
use "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_M_DeathRate2_reshaped.dta", clear

* Merge with Female dataset
merge 1:1 ID using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_F_DeathRate2_reshaped.dta"

* Drop merge indicators and unnecessary variables
drop _merge ID b_gender
sort past1 urban

* Save the merged dataset
save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_MF_DeathRate2_merged.dta", replace

* Export the selected columns to an Excel file
export excel using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_MF_DeathRate2_merged.xlsx", ///
    firstrow(variables) replace
	
	
* T-test
* Load the merged dataset
use "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_MF_DeathRate2_merged.dta", clear

* Initialize the Difference column
gen Difference = ""

* T-Test for "rate_female_01" between Rural and Urban when past1 == 0
ttest rate_female_01 if past1 == 0, by(urban)

* Store the difference and p-value from the t-test
local diff_0 = r(mu_1) - r(mu_2)
local pval_0 = r(p)

* Format the difference based on the p-value
if `pval_0' < 0.01 {
    local diff_0_str = "`=string(`diff_0', "%9.4f")'***"
}
else if `pval_0' <= 0.05 {
    local diff_0_str = "`=string(`diff_0', "%9.4f")'**"
}
...
* Update the Difference column for past1 == 0
replace Difference = "`diff_0_str'" if past1 == 0

* T-Test for "rate_female_01" between Rural and Urban when past1 == 1
ttest rate_female_01 if past1 == 1, by(urban)

* Store the difference and p-value from the t-test
local diff_1 = r(mu_1) - r(mu_2)
local pval_1 = r(p)

* Format the difference based on the p-value
if `pval_1' < 0.01 {
    local diff_1_str = "`=string(`diff_1', "%9.4f")'***"
}
else if `pval_1' <= 0.05 {
    local diff_1_str = "`=string(`diff_1', "%9.4f")'**"
}
...
* Update the Difference column for past1 == 1
replace Difference = "`diff_1_str'" if past1 == 1

* Display the updated dataset
list urban past1 rate_female_01 Difference

* Save the updated dataset
save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_MF_DeathRate2_ttest.dta", replace


* Export the selected columns to an Excel file
export excel using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\Sum_MF_DeathRate2_ttest.xlsx", ///
    firstrow(variables) replace
