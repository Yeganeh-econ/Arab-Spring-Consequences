...
\noindent \textbf{Circumcision Opposition (Binary).} Table~\ref{tab:circumcision} examines whether exposure to Arab Spring protests affected women's intentions regarding female genital cutting for their daughters. The dependent variable is binary and equals 0 if the respondent intends to circumcise her daughter (Yes) and 1 if the respondent reports either no intention to circumcise or is unsure (No/Unsure). Higher values therefore indicate greater opposition (or uncertainty) toward the practice.
...
\centering
\begin{threeparttable}
\caption{Treatment Effect on Intention to Circumcise (Binary Outcome)}
...
past\_group  & 0.095$^{***}$ & $-$0.023 & 0.054$^{***}$ & $-$0.007 & $-$0.011 \\
            & (0.019)       & (0.016)   & (0.016)       & (0.016)   & (0.016)   \\[3pt]
TC\_noborder & 0.009         & 0.023     & $-$0.006      & 0.024     & 0.021     \\
            & (0.019)       & (0.015)   & (0.017)       & (0.016)   & (0.017)   \\[4pt]
\midrule
...
\midrule
Observations & 27,392 & 27,392 & 27,174 & 27,366 & 27,113 \\
Adj.\ $R^2$  & 0.142  & 0.227  & 0.205  & 0.234  & 0.243  \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes}: Dependent variable is binary: 0 = respondent intends to circumcise her daughter (Yes), 1 = respondent reports no intention or is unsure (No/Unsure). Column (1) includes treatment variables only. Column (2) adds wealth quintiles, respondent education, age, and own circumcision status. Column (3) adds husband work status and education. Column (4) adds birth order and religion. Column (5) includes all controls. All specifications include area unit and birth year fixed effects; standard errors clustered at area unit level in parentheses. $^{***}$~$p<0.01$, $^{**}$~$p<0.05$, $^{*}$~$p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}

\noindent The estimated effect of protest exposure varies across specifications. In the baseline model (column 1), \texttt{past\_group} is positive and statistically significant (0.095, $p<0.01$), implying that higher exposure is associated with a higher probability of reporting No/Unsure (relative to intending to circumcise). Once socioeconomic controls and own circumcision status are included (column 2), the coefficient becomes negative and statistically insignificant ($-$0.023, $p=0.151$). When husband characteristics are included instead (column 3), \texttt{past\_group} is again positive and significant (0.054, $p<0.01$), but it is not statistically different from zero in the specifications that add birth order and religion and in the fully saturated model (columns 4--5). Across all columns, \texttt{TC\_noborder} is not statistically significant. Overall, the results suggest that any relationship between Arab Spring exposure and reported intention (No/Unsure vs Yes) is sensitive to included controls and may be mediated by correlated household and demographic characteristics.
...
% ============================================================
% SECTION 5: Mechanism Analysis: Shifting Gender Norms
% ============================================================
\noindent \textbf{Circumcision Opposition.} Table~\ref{tab:circumcision} examines whether exposure to Arab Spring protests affected women's intentions regarding female genital cutting. The dependent variable is an ordered measure of opposition to circumcision (0 = supports, 1 = unsure, 2 = opposes), where higher values indicate greater opposition to the practice.
