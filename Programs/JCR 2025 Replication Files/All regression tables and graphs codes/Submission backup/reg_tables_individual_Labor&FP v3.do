...
*======================================================================================
		...
		*use "tem_master_age1_UR_OLDSTATA", clear
...
if `table'==10.2 {	
	...
	*Model 1 Raw: (All dummies and interactons)
		...
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.3.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: logit visit_ternary past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.3.xls", append keep(past_group group past  $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.3.xls", append keep(work past_group group past  $xlist1) dec(3) pdec(3) e(r2_p)
...
if `table'==10.4 {	
	...
	*Model 1 Raw: (All dummies and interactons)
		...
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.2.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: logit pur_ternary past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.2.xls", append keep(past_group group past  $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.2.xls", append keep(work past_group group past  $xlist1) dec(3) pdec(3) e(r2_p)
...
if `table'==10.5 {	
	...
	*Model 1 Raw: (All dummies and interactons)
		...
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.1.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: probit health_ternary past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.1.xls", append keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA10.1.xls", append keep(work past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
...
* TABLE A10.6: All dummies regressions: * Dep Var: A Dummy Variable whether the women can make the decision about their own health logit
if `table'==10.6 {	
...
if `table'==11.1 {	
	...
	*Model 1 Raw: (All dummies and interactons)
		...
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.1.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: probit tol_vio_1 past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.1.xls", append keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.1.xls", append keep(work past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
...
* TABLE A11.2: All dummies regressions: * Dep Var: A Dummy Variable whether Domestic violence
if `table'==11.2 {	
	...
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		...
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA5.xls", replace keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p) 
		
	*Model 3: baseline: model 1 + governates_ur (`group' captured by governates_ur dummies) - absorb (clusterid) and group highly corralated
		quietly xi: logit tol_vio_1 past_group past group  $xlist1, cluster(clusterid)
		quietly est store model_3baseline
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA5.xls", append keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\AppendixTableA5.xls", append keep(work past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
...
if `table'==11.3 {	
	...
	*Model 1 Raw: (All dummies and interactons)
		...
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.3.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: probit tol_vio_noinfl_1 past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.3.xls", append keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.3.xls", append keep(work past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
...
* TABLE A11.4: All dummies regressions: * Dep Var: A Dummy Variable whether the women can make the decision about their own Domestic violence
if `table'==11.4 {	

	*Model 1 Raw: (All dummies and interactons)
		quietly xi: logit tol_vio_noinfl_1 past_group group past
		quietly est store model_raw
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.4.xls", replace keep(past_group group past) dec(3) pdec(3) e(r2_p)
			
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		quietly xi: logit tol_vio_noinfl_1 past_group group past  $xlist1
		quietly est store model_2
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.4.xls", append keep(past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p) 
	...
	*Model 5: model 3 + survey weighting
		...
		quietly est store m1_sw
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA11.4.xls", append keep(work past_group group past b_gender $xlist1) dec(3) pdec(3) e(r2_p)
