...
local table=9
				...
				/*				
				Table A2: Effect of Egyptian Protests on Missing Women (Full Table 5 Panel A in the Main Text)  - Baseline in Table 5 Panel A			 			    
				
				Table A3: Placebo effect of Egyptian Protests on Missing Women (Full Table 5 Panel B in the Main Text)		 	 		          - Placebo test on baseline regression in Table 5 Panel B
				
				Table A4: Different Phases of Egyptian Protests and Missing Women (Full Table 6 in the Main Text)
				
				Table A6: Sensitivity to Alternative Clustering Choices (Full Table 8 in the Main Text)
								
				Table A7: Robustness to Sample Selection (Full Table 9 in the Main Text)
				
				
				Table 8: Robustness Analysis                                         - Different subsamples and sample selections				
				*/
				
				local tab ="Main_appendix"
...
*               REGRESSION TABLES  : 
*======================================================================================	
* Table A2: Effect of Egyptian Protests on Missing Women (Full Table 5 Panel A in the Main Text)
	
if `table'==2 {
	
		*****************************************************	
		* TABLE A2 for Full Table 5 Panel A: models 1 to 6 (missing women estimations)
		***************************************************** 		
	...
	*Model 1 Raw: (only area group dummy, time dummy, and DID interaction term)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA2_`tab'_UR.xls", replace keep(past_group group past)
	...
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA2_`tab'_UR.xls", append keep(past_group group past $xlist) 
	...
	*Model 6: model 3 + self-created weighting (age, education, etc)
		...
		* The number of missing women per 100,000 women
		...
		save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA2_missing_women_appendix.dta", replace
		
}


* Table A3: Placebo effect of Egyptian Protests on Missing Women (Full Table 5 Panel B in the Main Text)
if `table'==3 {

		*****************************************************	
		* Table A3 in appendix for TABLE 5 Panel B: models 1 to 8 (placebo estimations(panel B))
		***************************************************** 
		preserve
		drop if year>2010
	...
	*Model 1 Raw: (only area group dummy, time dummy, and DID interaction term)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA3_`tab'_placebo.xls", replace keep(past_group_placebo group past_placebo)
	...
	*Model 2 Baseline: model 1 + all control variables (age, education, etc)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA3_`tab'_placebo.xls", append keep(past_group_placebo group past_placebo $xlist)
	...
	*Model 3: baseline: model 1 + governates_ur (`group' captured by governates_ur dummies)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA3_`tab'_placebo.xls", append keep(past_group_placebo past_placebo $xlist) 
	...
	*Model 6: model 3 + self-created weighting (age, education, etc)
		...
		* The number of missing women per 100,000 women
		...
		save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA3_missing_women_placebo_appendix.dta", replace
...
if `table'==4 {
		...
		*Phase 1 (incl. urban) + survey weighting		
			...
			*Treatment and control groups in phase1
			...
			quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA4_Phases_mw_appendix.xls", replace keep(past_group past $xlist)
		...
		*Phase 2 (incl. urban) + survey weighting
			...
			*Treatment and control groups in phase2
			...
			quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA4_Phases_mw_appendix.xls", append keep(past_group past $xlist)	
		...
		*Phase 4 (incl. urban) + survey weighting
			...
			*The number of missing women per 100,000 women
			...
			save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA4_missing_women_phases_appendix.dta", replace	
...
if `table'==6 {
	...
	*cluster: governorates_urban and rural; fixed effect: governorates_urban and rural (baseline, Tab 1, col 3)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",replace keep(past_group past $xlist) ctitle("Baseline")
	...
	*cluster: governorates_urban and rural; fixed effect: governorates (baseline, Tab 1, col 3)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",append keep(past_group past $xlist) ctitle("Clu: g_ur, FE: g")
	...
	*cluster: governorates; fixed effect: governorates_urban and rural (baseline, Tab 1, col 3)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",append keep(past_group past $xlist) ctitle("Clu: g, FE: g_ur")		
	...
	* BS800 cluster: governorates; fixed effect: governorates_urban and rural (baseline, Tab 1, col 3)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",append keep(past_group past $xlist) ctitle("BS800 Clu: g, FE: g_ur")		
	...
	*BS1000 cluster: governorates; fixed effect: governorates_urban and rural (baseline, Tab 1, col 3)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",append keep(past_group past $xlist) ctitle("BS1000 Clu: g, FE: g_ur")		
	...
	*Wild BS (Roodman et al 2018), cluster: governorates; fixed effect: governorates_urban and rural baseline, Tab 1, col 3)		
		...
		est store boot3
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_cluster_appendix.xls",append keep(past_group past $xlist) ctitle("wildBS Clu: g, FE: g_ur")		
		...
		*The number of missing women per 100,000 women
		...
		save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA6_missing_women_appendix.dta", replace
...
if `table'==7 {
	...
	*Model 1: Women aged 20-40. (Only 20 to 40 years old)
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA7_robustness_appendix.xls", replace keep(past_group past $xlist)
		restore
	...
	*Model 2: Interview: no husband. (without husband at the interview)
		...
		quietly outreg2 using "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA7_robustness_appendix.xls", append keep(past_group past $xlist)
		restore
	...
	*Model 6: No Cairo
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA7_robustness_appendix.xls", append keep(past_group group past $xlist) 
		restore	
	...
	*Model 8: Treatment and control groups by election results in 2012. 75% support of Moris
	...
	est tab sample2040 sample_noh sample_alone sample_border sample_SYPE sample_noCairo sample_noAlexandria sample_election75 , keep(past_group) title("`tab'") star(0.1 0.05 0.01) stats(N r2) b(%7.3f)
		...
		* The number of missing women per 100,000 women
		...
		save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA7_missing_women_robustness_appendix.dta", replace
		
		
}


*Table A8: Missing women in different age groups (Full Table 10 in the Main Text)
if `table'==8 {
		...
		*Model 1: age1 group - baseline result in table 5.1
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA8_different_ages_appendix.xls", replace keep(past_group group past $xlist)
		...
		*Model 2: age19 group		
		...
		quietly outreg2 using  "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA8_different_ages_appendix.xls",	append keep(past_group past $xlist)
		...
		* The number of missing women per 100,000 women
		...
		save "C:\Users\zheng\Dropbox\Papers_with_Zhengang\arab_spring_missing_women\results\TableA8_different_ages_appendix.dta", replace
